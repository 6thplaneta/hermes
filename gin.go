...
func ginLogger(c *gin.Context) {
	...
	if c.Request.Method != "GET" && level == logs.Trace &&
	...
	if logs.Instance().Level() == logs.Trace && data != "" {
		logs.Instance().Handle(logs.Instance().Level().NewWithTag(
			"Request", fmt.Sprintf("%3d | %v | %s | %s | %s\r\n%s",
